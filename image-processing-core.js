...
export function nearestColor(pixel, palette) {
...
export function quantizeWithoutDither(data, palette, minAlphaToProcess) {
  for (let i = 0; i < data.length; i += 4) {
...
export function quantizeWithFloydSteinberg(data, width, height, palette, config = DITHER_CONFIG) {
...
export function quantizeWithOrderedDither(data, width, height, palette, config = DITHER_CONFIG) {
